 ...
 */
import type { Baseline, MetricId, MetricSnapshot, TrendDirection } from '../types.js';
...
export function rollingBaseline(
  ...
  if (n < MIN_CONTRIBUTING_DAYS) return null;

  const mean = values.reduce((sum, v) => sum + v, 0) / n;
  const variance = values.reduce((sum, v) => sum + (v - mean) ** 2, 0) / (n - 1);
  const stddev = Math.sqrt(variance);

  return { metric, windowDays, mean, stddev, n };
}
 ...
 */
export function trend(series: DailyValue[], days: number): TrendDirection | null {
...
export function snapshot(
  ...
  const value = valueSeries[0]?.value ?? null;

  const baselineRange: DateRange = { from: addDays(date, -windowDays), to: addDays(date, -1) };
  ...
  const delta = value !== null && baseline !== null ? value - baseline.mean : null;
  const deltaPct =
    delta !== null && baseline !== null && baseline.mean !== 0 ? delta / baseline.mean : null;

  const trendRange: DateRange = { from: addDays(date, -(windowDays - 1)), to: date };
  const trendSeries = store.dailySeries(metric, trendRange);
  const trendDirection = trend(trendSeries, windowDays);

  return { metric, date, value, baseline, delta, deltaPct, trend: trendDirection };
}
