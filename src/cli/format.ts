 ...
 */
import type { Checkin, DailySummary, Flag, MetricSnapshot, SleepSession, SyncResult } from '../types.js';
...
function rhrPart(s: MetricSnapshot): string {
  if (s.value === null) return `RHR ${MISSING}`;
...
function hrvPart(s: MetricSnapshot): string {
  if (s.value === null) return `HRV ${MISSING}`;
  const glyph = arrow(s.trend);
...
export function formatBareSummary(s: DailySummary): string {
  const line = [rhrPart(s.rhr), hrvPart(s.hrv), sleepPart(s.sleep), spo2Part(s.spo2)].join('  ');
...
export function renderToday(
  ...
  if (opts.quiet) {
    return { output: hasFlags ? formatQuiet(summary) : '', exitCode: hasFlags ? 1 : 0 };
  }
  if (opts.json) {
    return { output: formatJson(summary), exitCode: 0 };
  }
  return { output: formatBareSummary(summary), exitCode: 0 };
}
...
export function formatSleepTable(sessions: SleepSession[]): string {
  if (sessions.length === 0) return 'No sleep sessions in range.';
  return sessions
    .map((s) => {
...
export function mergeDailySeries(rhr: DailyValue[], hrv: DailyValue[]): HeartRow[] {
...
export function formatHeartTable(rows: HeartRow[]): string {
  if (rows.length === 0) return 'No heart data in range.';
  return rows
    .map((r) => `${r.date}  RHR ${formatNumber(r.rhr, 0)}  HRV ${formatNumber(r.hrv, 0)}ms`)
    .join('\n');
}
...
export function mergeBodySeries(weight: DailyValue[], bodyFat: DailyValue[]): BodyRow[] {
  const dates = new Set<string>();
...
export function formatBodyTable(rows: BodyRow[]): string {
  if (rows.length === 0) return 'No body data in range.';
  return rows
    .map((r) => `${r.date}  ${formatNumber(r.weightKg, 1)}kg  ${formatNumber(r.bodyFatPct, 1)}% bf`)
    .join('\n');
}
...
export function formatPullResults(results: SyncResult[]): string {
  if (results.length === 0) return 'Nothing to sync.';
  return results
    .map((r) =>
      r.error
