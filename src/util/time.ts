...
const UTC_OFFSET_RE = /^(-?\d+(?:\.\d+)?)s$/;
const RELATIVE_DURATION_RE = /^(\d+)([dwm])$/;
...
export function parseUtcOffsetSeconds(
...
export function localDateOfSample(
  s: ApiSampleTime | undefined | null
): string | null {
  if (!s) {
    return null;
  }
...
export function localDateOfIntervalStart(
  i: ApiInterval | undefined | null
): string | null {
  if (!i) {
    return null;
  }
 ...
 */
export function localDateOfIntervalEnd(
  i: ApiInterval | undefined | null
): string | null {
  if (!i) {
    return null;
  ...
  return localDate(
    ...
    parseUtcOffsetSeconds(i.endUtcOffset ?? i.startUtcOffset)
  );
}
...
export function parseSince(input: string, now: Date = new Date()): Date {
  ...
  const rel = RELATIVE_DURATION_RE.exec(s);
