...
const CREDENTIALS_OR_TOKEN_RE = /credentials|token/i;
...
describe("createClient", () => {
    ...
    const fetchImpl = vi.fn(
      (url: string | URL | Request, init?: RequestInit) => {
        ...
        return Promise.resolve(
          jsonResponse(200, { dataPoints: [{ name: "a" }] })
        );
      }
    );
  ...
  it("pages() follows nextPageToken and sends it on the following call", async () => {
    ...
    const fetchImpl = vi.fn((url: string | URL | Request) => {
      ...
      if (String(url).includes("pageToken=tok-2")) {
        return Promise.resolve(
          jsonResponse(200, { dataPoints: [{ name: "page2" }] })
        );
      }
      return Promise.resolve(
        jsonResponse(200, {
          dataPoints: [{ name: "page1" }],
          nextPageToken: "tok-2",
        })
      );
    });
  ...
  it("retries after a 429 and succeeds on the next attempt", async () => {
    ...
    const fetchImpl = vi.fn(() => {
      ...
      return Promise.resolve(
        jsonResponse(200, { dataPoints: [{ name: "ok" }] })
      );
    });
    const delays: number[] = [];
  ...
  it("surfaces a 401 as API_HTTP with a credentials hint", async () => {
    ...
    expect((caught as VitalsError).hint).toMatch(CREDENTIALS_OR_TOKEN_RE);
  });
});
