...
import type { ApiDataPoint, ApiDataPointsResponse, DataTypeId } from '../../src/types.js';
import type { ListDataPointsOptions, VitalsApiClient } from '../../src/api/client.js';
...
function makeFakeClient(config: FakeClientConfig): VitalsApiClient {
  ...
  async function* pages(
    ...
    if (alwaysErr) throw alwaysErr;

    if (o.filter !== undefined && config.throwOnFilter?.has(id)) {
      throw new VitalsError('API_FILTER_UNSUPPORTED', `Data type "${id}" rejects filters.`);
    }
...
describe('syncAll', () => {
  it('stops paging early once a page predates the window by more than 2 hours', async () => {
    ...
    const [result] = await syncAll(store, client, { since, dataTypes: ['heart-rate'] });
  ...
  it('falls back to unfiltered paging when the API rejects the date filter', async () => {
    const since = new Date('2026-08-01T00:00:00Z');
    const unfilteredPage = page([weightPoint('2026-08-03T08:00:00Z', 70000)]);
    ...
    const [result] = await syncAll(store, client, { since, dataTypes: ['weight'] });
  ...
  it('does not advance the watermark for a failed data type', async () => {
    ...
    await syncAll(store, client, { since, dataTypes: ['weight'] });
  ...
  it('advances the watermark to the newest timestamp actually persisted', async () => {
    const since = new Date('2026-08-01T00:00:00Z');
    const p1 = page([weightPoint('2026-08-02T09:00:00Z', 70100)]);
    const p2 = page([weightPoint('2026-08-04T09:00:00Z', 70200)]); // newest
  ...
  it('is idempotent across two runs over the same canned pages', async () => {
    const since = new Date('2026-08-01T00:00:00Z');
    const pages = [
      page([weightPoint('2026-08-02T09:00:00Z', 70100)]),
      page([weightPoint('2026-08-04T09:00:00Z', 70200)]),
    ];
