...
import { nutritionLogSpec, hydrationLogSpec } from '../../src/datatypes/logs.js';
...
describe('nutritionLogSpec', () => {
  ...
  it('supports date filtering', () => {
    ...
    expect(nutritionLogSpec.filterField).toBe('nutritionLog.interval.start_time');
  });
  ...
  it('missing/zero energy resolves to null or a plausible number, never NaN', () => {
    ...
    for (const entry of batch.nutrition) {
      if (entry.energyKcal === null) continue;
  ...
  it('natural key combines start instant and food display name', () => {
    ...
    const batch = nutritionLogSpec.parse([point]);
    expect(batch.nutrition[0]!.naturalKey).toBe('2026-01-01T10:00:00.000Z|Apple');
  });
...
describe('hydrationLogSpec', () => {
  ...
  it('supports date filtering', () => {
    ...
    expect(hydrationLogSpec.filterField).toBe('hydrationLog.interval.start_time');
  });
  ...
  it('naturalKey is the normalized interval start instant', () => {
    ...
    expect(batch.hydration[0]!.naturalKey).toBe('2026-01-01T10:00:00.000Z');
    expect(batch.hydration[0]!.milliliters).toBe(250);
  });
