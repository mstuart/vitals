...
describe("distanceSpec", () => {
  ...
  it("converts millimeters to metres, and daily distance is plausible", () => {
    ...
    const raw = (
      points[0]?.distance as { millimeters?: number | string } | undefined
    )?.millimeters;
...
describe("activeEnergyBurnedSpec", () => {
  ...
  it("supports date filtering", () => {
    ...
    expect(activeEnergyBurnedSpec.filterField).toBe(
      "activeEnergyBurned.interval.start_time"
    );
  });
  ...
  it("handles string kcal values without crashing", () => {
    ...
    expect(batch.observations.at(0)?.value).toBe(1.5);
  });
});
...
describe("activeZoneMinutesSpec", () => {
  ...
  it("supports date filtering", () => {
    ...
    expect(activeZoneMinutesSpec.filterField).toBe(
      "activeZoneMinutes.interval.start_time"
    );
  });
