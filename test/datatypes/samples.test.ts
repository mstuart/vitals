...
describe("heartRateSpec", () => {
  ...
  it("every bucket has min <= avg <= max and a positive sample count", () => {
    ...
    for (const bucket of batch.heartRateHourly) {
      ...
      expect(bucket.date).toMatch(ISO_DATE_RE);
    }
  });
  ...
  it("produces identical bucket natural keys across two parses of the same fixture", () => {
    ...
    const keys1 = batch1.heartRateHourly
      .map((b) => b.naturalKey)
      .sort((a, b) => a.localeCompare(b));
    const keys2 = batch2.heartRateHourly
      .map((b) => b.naturalKey)
      .sort((a, b) => a.localeCompare(b));
  ...
  it("never throws on malformed points", () => {
    const malformed: ApiDataPoint[] = [
      {},
      { heartRate: {} },
      { heartRate: { beatsPerMinute: "84", sampleTime: {} } },
      {
        heartRate: {
          beatsPerMinute: "84",
          sampleTime: { physicalTime: "not-a-date" },
        },
      },
      {
        heartRate: {
          beatsPerMinute: "NaN",
          sampleTime: { physicalTime: "2026-08-05T19:00:00Z" },
        },
      },
...
describe("heartRateVariabilitySpec", () => {
  ...
  it("handles empty input and malformed points without throwing", () => {
    ...
    expect(heartRateVariabilitySpec.parse(malformed).observations).toHaveLength(
      0
    );
  });
});
...
describe("impossible zero readings are treated as missing", () => {
  it("drops a zero RMSSD from heart-rate-variability", () => {
    const point = {
      ...
      heartRateVariability: {
        ...
        sampleTime: {
          physicalTime: "2026-08-06T09:00:00Z",
          utcOffset: "-25200s",
        },
      },
  ...
  it("drops a zero bpm rather than dragging the hourly average down", () => {
    const zero = {
      heartRate: {
        beatsPerMinute: "0",
        sampleTime: { physicalTime: "2026-08-06T09:00:00Z" },
      },
    } as unknown as ApiDataPoint;
    const real = {
      heartRate: {
        beatsPerMinute: "60",
        sampleTime: { physicalTime: "2026-08-06T09:01:00Z" },
      },
    } as unknown as ApiDataPoint;
    const buckets = heartRateSpec.parse([zero, real]).heartRateHourly;
    expect(buckets).toHaveLength(1);
    expect(buckets.at(0)?.sampleCount).toBe(1);
    expect(buckets.at(0)?.avgBpm).toBe(60);
    expect(buckets.at(0)?.minBpm).toBe(60);
  });
